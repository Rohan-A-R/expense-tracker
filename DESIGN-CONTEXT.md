...
**AI Analysis (stocks only, between ABOUT and NEWS):** styled as an ordinary section of the page, not a widget — no tinted card, no coloured rail, no emoji. Header "AI ANALYSIS" uses the same small-caps + ink rule as ABOUT/NEWS, with a rust **REFRESH** link at the right once a report exists. Idle: one line describing the note, an optional "Last analysed 16 Sept — reports are kept for 7 days", and a full-width **solid ink "Analyse this stock"** button. Running: the live step in italic serif ("Loading Steel sector context…", "Writing the analysis…"). The report: **SCORE** as a large serif numeral (`52 / 100`) with **VERDICT** in coloured small caps at the right — green `#4E9E6A` BUY / amber `#B5761F` WATCH / rust `#D9481C` AVOID — over a flat 4px score bar (square ends, like the budget bar), then an italic-serif headline. **Your holding** uses the Home stat-row language: ink top rule, dotted dividers, three cells YOUR P&L (green/rust) · OF PORTFOLIO · AVG COST, followed by the model's note and a muted "Reference levels · support ₹… · 2×ATR ₹…. Not a sell signal." Then small-caps paragraphs TECHNICALS · VALUATION · RESULTS & EARNINGS · SECTOR & DRIVERS · KEY LEVELS, **CATALYSTS** (green label) and **RISKS** (rust label) as em-dash lists with dotted dividers, VERDICT, and a dotted-rule footer (time · N peers · N market drivers · N headlines · not investment advice). Below, **PAST CALLS**: one row per call — date · verdict (coloured small caps) · score · price then → now · % move (green/rust, or "today" for a fresh call).
