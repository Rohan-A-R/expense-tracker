...
**AI Analysis (stocks only, between ABOUT and NEWS):** an idle card in the AI style (paper, soft purple tint, 3px purple `#6C5FB0` left rail, "AI ANALYSIS 🔬" label, purple **🔬 Analyse this stock** pill). While running it shows bouncing purple dots + the live step ("Loading Steel sector context…", "Reading the news…", "Writing the analysis…"). The finished report: **SCORE** in large serif (`52/100`) coloured by verdict — green `#4E9E6A` BUY / amber `#C9972E` WATCH / rust `#D9481C` AVOID — beside a filled verdict chip, a thin score bar, then a serif headline. Next a **YOUR HOLDING** box (faint ink fill, hairline border): three mini stats P&L (green/rust) · Of portfolio · Avg cost, the model's note to the holder, and a muted "Reference levels · support ₹… · 2×ATR ₹… — not a sell signal". Then small-caps sections TECHNICALS · VALUATION · RESULTS & EARNINGS · SECTOR & DRIVERS · KEY LEVELS, **CATALYSTS** (green ▲ bullets), **RISKS** (rust ▼ bullets), VERDICT, an optional "Data not available: …" line, and a footer "Generated … from N peers, N market drivers and N headlines · Research and education only — not investment advice." A purple **↻ REFRESH** sits top-right.
