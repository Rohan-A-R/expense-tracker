...
**Chart support/resistance (no AI):** `HoldingDetail.jsx` draws one support and one resistance on the price chart for **1M · 6M · 1Y · 5Y** (not MAX), each computed from *that range's own bars* in `stockCache` via `keyLevels` (`technicals.js`) with per-range `LEVEL_CFG` (pivot width `k`, max distance from price). `levels()` clusters swing highs and lows together (role reversal), with an ATR-scaled merge tolerance, and reports touches; `keyLevels` prefers a level tested 2+ times, then the nearest. The AI report's levels use the same function on 1Y daily bars, so the chart's 1Y view matches the report. Values are printed in a key under the chart, never on it — the nearest levels always sit beside the latest price at the right edge, so an in-chart label lands on the price line. Toggle persisted as the `chartLevels` setting (on by default).

**"Today" on the holding screen:** for stocks, `prevClose` comes from the daily quote in the price cache (or the previous 1Y bar), **not** `stats.prevClose` — `stats` is the selected range's chart meta, whose previous close is the one before the whole range, so "today" used to change as you tapped range pills.
